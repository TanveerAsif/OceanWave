					...
					if (hr == S_OK)
					{
						...
						if (hr == S_OK)
						{
							...
							if (hr == S_OK)
							{
								...
								D3DXMatrixPerspectiveFovLH(&m_matProjection, fieldOfView, screenAspect, 0.1f, 10000.0f);


								D3DXMatrixOrthoLH(&m_matOrtho, (float)screenWidth, (float)screenHeight, 0.1, 10000.0f);
								...
								D3D11_BLEND_DESC blendState2Desc;
								ZeroMemory(&blendState2Desc, sizeof(blendState2Desc));
								blendState2Desc.RenderTarget[0].BlendEnable = TRUE;
								blendState2Desc.RenderTarget[0].SrcBlend = D3D11_BLEND_ONE;
								blendState2Desc.RenderTarget[0].DestBlend = D3D11_BLEND_ONE;
								blendState2Desc.RenderTarget[0].BlendOp = D3D11_BLEND_OP_ADD;
								blendState2Desc.RenderTarget[0].SrcBlendAlpha = D3D11_BLEND_ONE;
								blendState2Desc.RenderTarget[0].DestBlendAlpha = D3D11_BLEND_ZERO;
								blendState2Desc.RenderTarget[0].BlendOpAlpha = D3D11_BLEND_OP_ADD;
								blendState2Desc.RenderTarget[0].RenderTargetWriteMask = 0x0f;
								hr = m_pDevice->CreateBlendState(&blendState2Desc, &m_pBlendState2);
								if (hr == S_OK)
								{
...
void Dx11_Direct3D::EnableBlendState2()
{
	if (m_pBlendState2)
		m_pDeviceContext->OMSetBlendState(m_pBlendState2, 0, 0xffffffff);
}
